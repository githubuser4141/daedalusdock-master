...
/mob/living/carbon/human/get_edge_protection(def_zone)
	...
	if(def_zone)
		if(isbodypart(def_zone))
			...
			affecting = get_bodypart(deprecise_zone(def_zone))
...
/mob/living/carbon/human/proc/getallsubarmor(d_type)
	...
	var/static/list/converstion_table = list(BLUNT, PUNCTURE)
...
/mob/living/carbon/human/damage_armor(damage = 0, damage_flag = BLUNT, damage_type = BRUTE, sharpness = NONE, def_zone = BODY_ZONE_CHEST)
