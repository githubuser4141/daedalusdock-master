...
/datum/unit_test/ms13_terrain_hiveminds/Run()
	...
	living_host.set_stat(UNCONSCIOUS)
	...
	TEST_ASSERT_EQUAL(xeno_network.get_corpse_claim(living_host), xeno_network.core, "The delivered living host was not reserved by the xenomorph nest.")
	var/old_xeno_unit_count = length(xeno_network.units)
	xeno_network.process_structure_corpse(xeno_network.core, xeno_network.structure_conversion_time)
	TEST_ASSERT(!QDELETED(living_host) && living_host.stat == DEAD, "A completed xenomorph incubation did not leave a fatally chest-wounded host body.")
