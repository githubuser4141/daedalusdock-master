...
/datum/pathogen/pocklung
	...
	pathogen_flags = PATHOGEN_SPREAD_NON_CONTAGIOUS
	max_stages = 4
...
/datum/pathogen/pocklung/on_infect_mob(mob/living/infectee, make_copy)
	. = ..()
	...
	switch(stage)
		if(1)
			if(DT_PROB(2, delta_time))
				...
				affected_mob.adjust_jitter(0.1 SECONDS)
				affected_mob.adjustOrganLoss(ORGAN_SLOT_LUNGS, (0.1 * infection_purity), 100)
		if(2)
			if(DT_PROB(4, delta_time))
				affected_mob.emote("cough")
				affected_mob.adjust_jitter(0.2 SECONDS)
