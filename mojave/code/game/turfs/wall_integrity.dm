...
/// Walls take bullet damage like /obj/bullet_act() does; how much reaches them is decided in
/// penetrating_hit() (bullet_math.dm), same as for everything else.
/turf/closed/wall/bullet_act(obj/projectile/hitting_projectile, def_zone, piercing_hit = FALSE)
	...
	if(QDELETED(src) || . != BULLET_ACT_HIT)
		return
	...
	if(hitting_projectile.firer)
		log_combat(hitting_projectile.firer, src, "shot", hitting_projectile, "[round(hitting_projectile.damage, 0.1)] damage")
