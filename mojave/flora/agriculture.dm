...
/**
 * Adjust Health.
 * Raises the tray's plant_health stat by a given amount, with total health determined by the seed's base_endurance.
...
/obj/machinery/ms13/agriculture/CtrlClick(mob/user)
	. = ..()
	// canUseTopic now takes a single bitflag arg instead of positional booleans; NO_TK -> USE_IGNORE_TK
