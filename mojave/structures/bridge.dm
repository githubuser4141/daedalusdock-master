TYPEINFO_DEF(/obj/structure/ms13/bridge)
	default_armor = list(BLUNT = 50, PUNCTURE = 10, SLASH = 50, LASER = 0, ENERGY = 0, BOMB = 0, BIO = 0, FIRE = 0, ACID = 0)
...
/obj/structure/ms13/bridge/update_overlays()
	...
	// ponytail: dropped `plane = GAME_PLANE_UPPER` on the overchar overlays below - that plane doesn't exist
	// here. layer=ABOVE_MOB_LAYER on GAME_PLANE (the default) should still draw them over mobs; if the rope
	// ever renders under a mob standing on the bridge, a real upper plane + plane master needs adding.
	...
	if (dir == WEST || dir == EAST)
		. += mutable_appearance(icon, "rope_overchar_[dir]", layer=ABOVE_MOB_LAYER)
	else
		var/turf/turf_south = get_step(src, SOUTH)
		var/end = locate(/obj/structure/ms13/bridge_stakes) in turf_south
		if(end)
			. += mutable_appearance(icon, "rope_overchar_special", layer=ABOVE_MOB_LAYER)
		else
			. += mutable_appearance(icon, "rope_overchar_[dir]", layer=ABOVE_MOB_LAYER)
...
TYPEINFO_DEF(/obj/structure/ms13/bridge_stakes)
	default_armor = list(BLUNT = 50, PUNCTURE = 0, SLASH = 50, LASER = 0, ENERGY = 0, BOMB = 0, BIO = 0, FIRE = 0, ACID = 0)
...
/obj/structure/ms13/bridge_stakes/update_overlays()
	...
	. += mutable_appearance('mojave/icons/structure/bridge.dmi', "rope_overchar_end_[dir]", layer=ABOVE_MOB_LAYER)
	...
	. += mutable_appearance(icon, "stake_overchar_[dir]", layer=ABOVE_MOB_LAYER)
